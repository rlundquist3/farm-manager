...
share.Route = (function() {                                                                                          // 1
  ...
  Route.prototype._callEndpoint = function(endpointContext, endpoint) {                                              // 3
    if (this._authAccepted(endpointContext, endpoint)) {                                                             // 147
      if (this._roleAccepted(endpointContext, endpoint)) {                                                           // 148
        ...
        return {                                                                                                     //
          ...
          body: {                                                                                                    // 151
            status: 'error',                                                                                         // 152
            message: 'You do not have permission to do this.'                                                        // 152
          }                                                                                                          //
        };                                                                                                           //
      }                                                                                                              //
    } else {                                                                                                         //
      return {                                                                                                       //
        ...
        body: {                                                                                                      // 154
          status: 'error',                                                                                           // 155
          message: 'You must be logged in to do this.'                                                               // 155
        }                                                                                                            //
      };                                                                                                             //
    }                                                                                                                //
  };                                                                                                                 //
  ...
  Route.prototype._authenticate = function(endpointContext) {                                                        // 3
    ...
    auth = this.api._config.auth.user.call(endpointContext);                                                         // 182
    if ((auth != null ? auth.userId : void 0) && (auth != null ? auth.token : void 0) && !(auth != null ? auth.user : void 0)) {
      userSelector = {};                                                                                             // 186
      userSelector._id = auth.userId;                                                                                // 186
      userSelector[this.api._config.auth.token] = auth.token;                                                        // 186
      auth.user = Meteor.users.findOne(userSelector);                                                                // 186
    }                                                                                                                //
    if (auth != null ? auth.user : void 0) {                                                                         // 192
      endpointContext.user = auth.user;                                                                              // 193
      endpointContext.userId = auth.user._id;                                                                        // 193
      return true;                                                                                                   //
    } else {                                                                                                         //
      return false;                                                                                                  //
    }                                                                                                                //
  ...
  Route.prototype._roleAccepted = function(endpointContext, endpoint) {                                              // 3
    if (endpoint.roleRequired) {                                                                                     // 208
      if (_.isEmpty(_.intersection(endpoint.roleRequired, endpointContext.user.roles))) {                            // 209
        return false;                                                                                                // 210
      }                                                                                                              //
    }                                                                                                                //
    return true;                                                                                                     //
  ...
  Route.prototype._respond = function(response, body, statusCode, headers) {                                         // 3
    ...
    defaultHeaders = this._lowerCaseKeys(this.api._config.defaultHeaders);                                           // 220
    headers = this._lowerCaseKeys(headers);                                                                          // 220
    headers = _.extend(defaultHeaders, headers);                                                                     // 220
    if (headers['content-type'].match(/json|javascript/) !== null) {                                                 // 225
      if (this.api._config.prettyJson) {                                                                             // 226
        body = JSON.stringify(body, void 0, 2);                                                                      // 227
      } else {                                                                                                       //
        body = JSON.stringify(body);                                                                                 // 229
      }                                                                                                              //
    }                                                                                                                //
    sendResponse = function() {                                                                                      // 220
      response.writeHead(statusCode, headers);                                                                       // 233
      response.write(body);                                                                                          // 233
      return response.end();                                                                                         //
    };                                                                                                               //
    if (statusCode === 401 || statusCode === 403) {                                                                  // 236
      minimumDelayInMilliseconds = 500;                                                                              // 243
      randomMultiplierBetweenOneAndTwo = 1 + Math.random();                                                          // 243
      delayInMilliseconds = minimumDelayInMilliseconds * randomMultiplierBetweenOneAndTwo;                           // 243
